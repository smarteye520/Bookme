...
#include <openssl-1.1.0f/include/openssl/aes.h>

char * get_products_db_path()
{
    char * productsPath = malloc(strlen(PATH_BASE) + strlen(PRODUCTS_DB_NAME) + 1);
...
{
    char * productPath = malloc(strlen(PATH_BASE) + strlen(danacode) + strlen(DB_EXT) + 1);
...
static int database_callback2(void *data, int argc, char **argv, char **azColName){
    int i;
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "database_callbak2_called",1);

    for(i = 0; i<argc; i++){
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "%s = ", azColName[i]);
...
char * get_create_product_table_sql(void)
{
    ...
    char* csql;
    csql = malloc(strlen(sql1) + strlen(PRODUCTS_TABLE_NAME) + strlen(sql2)
                  ...
                  + strlen(PRODUCTS_FIELD_NAME_PARAM10) + strlen(sql5)  + strlen(sql6) + 1);
    strcpy(csql, sql1);
...
char * get_create_files_table_sql(void)
{
    ...
    char* csql;
    csql = malloc(strlen(sql1) + strlen(FILES_TABLE_NAME) + strlen(sql2)
                  ...
                  + strlen(FILES_FIELD_NAME_PARAM2) + strlen(sql5)+ strlen(sql6) + 1);
    strcpy(csql, sql1);
...
char * get_delete_files_table_sql(void)
{
    ...
    char* csql;
    csql = malloc(strlen(sql1) + strlen(FILES_TABLE_NAME) + 1);
    strcpy(csql, sql1);
    strcat(csql, FILES_TABLE_NAME);
    return csql;
}

char * get_add_new_product_sql()
{
    ...
    csql = malloc(strlen(sql1) + strlen(PRODUCTS_TABLE_NAME) + strlen(sql2)
                  ...
                  //+ strlen(sql6) + strlen(sql9) + strlen(sql6) + strlen(sql3)
                  ...
                  + strlen(PRODUCTS_FIELD_NAME_KEYTOKEN_PARAMETER) + strlen(sql5) + strlen(sql7) + 1);
    strcpy(csql, sql1);
    strcat(csql, PRODUCTS_TABLE_NAME);
    strcat(csql, sql2);
    strcat(csql, PRODUCTS_FIELD_NAME_DATANCODE);
...
char * get_add_new_section_sql()
{
    ...
    csql = malloc(strlen(sql1) + strlen(FILES_TABLE_NAME) + strlen(sql2)
                  ...
                  + strlen(FILES_FIELD_NAME_VALUE_PARAMETER) + strlen(sql5) + strlen(sql7) + 1);
    strcpy(csql, sql1);
    strcat(csql, FILES_TABLE_NAME);
    strcat(csql, sql2);
    strcat(csql, FILES_FIELD_NAME_TYPE);
...
char * get_select_product_sql_all_columns()
{
    char *sql3 = " , ";
    char* csql;
    csql = malloc(strlen(PRODUCTS_FIELD_NAME_ID) + strlen(sql3)
                  + strlen(PRODUCTS_FIELD_NAME_DATANCODE) + strlen(sql3)
                  + strlen(PRODUCTS_FIELD_NAME_BOOKNAME) + strlen(sql3)
                  + strlen(PRODUCTS_FIELD_NAME_COVER) + strlen(sql3)
                  ...
                  + strlen(PRODUCTS_FIELD_NAME_PARAM8) + strlen(sql3)
                  + strlen(PRODUCTS_FIELD_NAME_PARAM9) + strlen(sql3)
                  + strlen(PRODUCTS_FIELD_NAME_PARAM10) + 1);
...
char * get_select_products_sql(int mode)
{
    ...
    if (mode == 3) {
        ...
        sql5 = malloc(strlen(sql5a));
        strcpy(sql5,sql5a);
    }

    char* csql;
    csql = malloc(strlen(sql1) + strlen(sql5) + strlen(sql2) + strlen(PRODUCTS_TABLE_NAME) + 1);
    strcpy(csql, sql1);
    strcat(csql, sql5);
    free(sql5);
    strcat(csql, sql2);
...
char * get_select_product_sql_by_danacode(int mode)
{
    ...
    char* csql;
    csql = malloc(strlen(sql1) + strlen(sql5) + strlen(sql2)
                  + strlen(PRODUCTS_TABLE_NAME) + strlen(sql3)
                  + strlen(PRODUCTS_FIELD_NAME_DATANCODE) + strlen(sql4)
                  + strlen(PRODUCTS_FIELD_NAME_DANACODE_PARAMETER) + 1);
    strcpy(csql, sql1);
    strcat(csql, sql5);
    free(sql5);
    strcat(csql, sql2);
...
char * get_select_save_chapter()
{
    ...
    char* csql;
    csql = malloc(strlen(sql1) + strlen(PRODUCTS_FIELD_NAME_SAVE_CHAPTER) + strlen(sql2)
                  + strlen(PRODUCTS_TABLE_NAME) + strlen(sql3)
                  + strlen(PRODUCTS_FIELD_NAME_DATANCODE) + strlen(sql4)
                  + strlen(PRODUCTS_FIELD_NAME_DANACODE_PARAMETER) + 1);
    strcpy(csql, sql1);
...
char * get_select_section_sql(void)
{
    char *sql1 = "SELECT ";
    char *sql2 = " FROM ";
    char *sql3 = " * ";
    ...
    char* csql;
    csql = malloc(strlen(sql1) + strlen(sql3) + strlen(sql2)
                  + strlen(FILES_TABLE_NAME) + 1);
    strcpy(csql, sql1);
    strcat(csql, sql3);
    strcat(csql, sql2);
    strcat(csql, FILES_TABLE_NAME);
...
char * get_delete_product_sql_by_danacode(void)
{
    ...
    char *sql4 = "=";
    char* csql;
    csql = malloc(strlen(sql1) + strlen(sql2)
                  + strlen(PRODUCTS_TABLE_NAME) + strlen(sql3)
                  + strlen(PRODUCTS_FIELD_NAME_DATANCODE) + strlen(sql4)
                  + strlen(PRODUCTS_FIELD_NAME_DANACODE_PARAMETER) + 1);
    strcpy(csql, sql1);
...
char * get_select_content_sql()
{
    ...
    char* csql;
    csql = malloc(strlen(sql1) + strlen(FILES_FIELD_NAME_VALUE)
                  ...
                  + strlen(FILES_FIELD_NAME_TYPE_NUM_PARAMETER) + 1
    );
    strcpy(csql, sql1);
    strcat(csql, FILES_FIELD_NAME_VALUE);
    strcat(csql, sql6);
...
char * get_select_product_details_sql()
{
    char *sql1 = "SELECT ";
    char *sql2 = " FROM ";
    char *sql3 = " WHERE ";
    ...
    char* csql;
    csql = malloc(strlen(sql1) + strlen(PRODUCTS_FIELD_NAME_KEYTOKEN) + strlen(sql6)
                  + strlen(PRODUCTS_FIELD_NAME_BOOKNAME)  + strlen(sql2) + strlen(PRODUCTS_TABLE_NAME)
                  + strlen(sql3)
                  + strlen(PRODUCTS_FIELD_NAME_DATANCODE) + strlen(sql4)
                  + strlen(PRODUCTS_FIELD_NAME_DANACODE_PARAMETER) + 1
    );
...
char * get_update_save_position_chapter()
{
    char *sql1 = "UPDATE ";
    char *sql2 = " SET ";
    char *sql3 = " = ";
...
int set_product(const unsigned char * danacode, const unsigned char * bookname,
                ...
                const unsigned char * numOfCharacters)
{
...
int set_sections(const unsigned char * danacode, cJSON * json)
{
    ...
    int i_numOfSection = numOfSection->valueint;
...
{
    char * productPath = malloc(strlen(PATH_BASE) + strlen(PRODUCTS_DB_NAME) + 1);
    strcpy(productPath, PATH_BASE);
    strcat(productPath, PRODUCTS_DB_NAME);
    ...
    free(sql3);
    if( rc != SQLITE_OK ){
        LogString("update_chapter_position rc != SQLITE_OK",zErrMsg);
        sqlite3_free(zErrMsg);
        return;
    }
    ...
    sqlite3_bind_text(res2, idx, danacode, strlen(danacode), 0);
    sqlite3_step(res2);
    LogInt("new position", position);
    LogString("sql command ", sqlite3_expanded_sql(res2));
    sqlite3_finalize(res2);
    sqlite3_close(db);
}
...
char * get_product_content(char * danacode, int chapter, char ** outTitle)
{
    ...
    //LogString("sql command ", sqlite3_expanded_sql(res));
    unsigned char * content = "";
    if (step == SQLITE_ROW) {
        unsigned char * content2 = sqlite3_column_text(res,0);
        content = malloc(strlen(content2) + 1);
        strcpy(content, content2);

        unsigned char * title2 = sqlite3_column_text(res,1);
        *outTitle = malloc(strlen(title2) + 1);
...
char * get_select_toc_sql()
{
    ...
    char* csql;
    csql = malloc(strlen(sql1) + strlen(FILES_FIELD_NAME_TYPE_NUM) + strlen(sql6)
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "CCMMDD: SQL SYNTAX: SELECT_PRODUCT_SQL: %s", csql);
...
char * get_product_toc(char * danacode)
{
    ...
    char * jsonSerialize = cJSON_Print(json);
    LogString("jsonSerialize", jsonSerialize);
    cJSON_Delete(json);
    return jsonSerialize;
}
